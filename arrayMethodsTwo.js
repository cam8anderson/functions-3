...
// CODE HERE
const numbersMap = numbers.map((num) => num / 2);
...
const endsInI = cities.filter((elem) => elem[elem.length - 1] === "i");
...
const fruitSort = fruits.forEach((elem) => {
  if (elem.color === "red") {
    console.log(`The fruit with index ${fruits.indexOf(elem)} is an apple.`);
  } else if (elem.color === "orange") {
    console.log(`The fruit with index ${fruits.indexOf(elem)} is an orange.`);
  } else {
    console.log(
      `The fruit with index ${fruits.indexOf(elem)} is neither apple or orange.`
    );
  }
});
